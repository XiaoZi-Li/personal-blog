...
export default function Home() {
  ...
  const skillCategories = [
    ...
    },
    {
      ...
      icon: Cpu,
      skills: [
        { name: 'FPGA', level: '熟悉应用' },
        { name: 'ESP32', level: '核心掌握' },
        { name: 'STM32', level: '熟悉应用' },
        { name: '鸿蒙Hi3861', level: '熟悉应用' },
      ]
    },
    {
      title: 'AI技术栈',
      icon: Brain,
      skills: [
        { name: 'RAG技术', level: '核心掌握' },
        { name: 'MCP协议', level: '核心掌握' },
        { name: 'AI Skills', level: '学习进阶' },
      ]
    },
  ];
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.skills.map((skill, j) => {
                      ...
                      const getSkillVariant = (level?: string) => {
                        if (!level) return 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300';
                        if (level === '核心掌握') return 'bg-gradient-to-r from-green-100 to-emerald-100 dark:from-green-900/50 dark:to-emerald-900/50 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800';
                        if (level === '熟悉应用') return 'bg-gradient-to-r from-blue-100 to-cyan-100 dark:from-blue-900/50 dark:to-cyan-900/50 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800';
                        if (level === '学习进阶') return 'bg-gradient-to-r from-amber-100 to-yellow-100 dark:from-amber-900/50 dark:to-yellow-900/50 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800';
                      ...
                      return (
                        <Badge 
                          key={j} 
                          variant="secondary"
                          className={`px-3 py-1 hover:scale-105 transition-transform ${getSkillVariant(skill.level)}`}
                        >
                          {skill.name}
      ...
      <section className="py-16 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground">竞赛经历与荣誉奖项</p>
          </div>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {[
                { 
                  ...
                  awards: ['全国大学生嵌入式芯片与系统设计竞赛', '中国大学生服务外包创新创业大赛']
                },
                { 
                  title: '省级一等奖',
                  count: 1, 
                  color: 'from-blue-500 to-cyan-500',
                  awards: ['天津市"新工科"工程实践创新技术竞赛']
                },
                { 
                  title: '总计获奖',
                  count: 3, 
                ...
                <div key={i} className="text-center">
                  <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${item.color} flex items-center justify-center mx-auto mb-3 shadow-lg`}>
                    <Trophy className="w-10 h-10 text-white" />
                  </div>
                  <div className="text-3xl font-bold text-slate-800 dark:text-slate-100">{item.count}</div>
                  <div className="text-sm text-muted-foreground">{item.title}</div>
                  {item.awards.length > 0 && (
                    <div className="mt-2 text-xs text-slate-500 dark:text-slate-400 max-w-32">
                      {item.awards.map((award, j) => (
                        <div key={j} className="truncate" title={award}>{award}</div>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3 className="font-bold text-slate-800 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors mb-1">
                        {proj.title}
                      </h3>
            ...
            <CardContent className="pt-8">
              <div className="grid sm:grid-cols-2 gap-6">
                {[
                  ...
                  { title: '未来方向', desc: '计划深入研究具身智能在工业场景的应用，探索AI Agent在机器人系统中的集成方案' },
                  { title: '正在学习', desc: '深入学习Rust语言在嵌入式开发中的应用，探索dora-rs框架的高级特性' },
                ].map((item, i) => (
              ...
              {/* 求职意向 */}
              <div className="mt-8 p-6 rounded-xl bg-gradient-to-r from-violet-100 to-purple-100 dark:from-violet-950/50 dark:to-purple-950/50">
                <h3 className="font-bold text-lg mb-4 text-slate-800 dark:text-slate-100">求职意向</h3>
                <div className="grid sm:grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-slate-600 dark:text-slate-400 mb-1">期望职位</p>
                    <p className="font-medium text-slate-800 dark:text-slate-200">嵌入式开发工程师 / 具身智能工程师</p>
                  </div>
                  <div>
                    <p className="text-slate-600 dark:text-slate-400 mb-1">期望城市</p>
                    <p className="font-medium text-slate-800 dark:text-slate-200">北京 / 天津 / 深圳</p>
                  </div>
                  <div>
                    <p className="text-slate-600 dark:text-slate-400 mb-1">入职时间</p>
                    <p className="font-medium text-slate-800 dark:text-slate-200">随时可到岗</p>
                  </div>
                  <div>
                    <p className="text-slate-600 dark:text-slate-400 mb-1">期望薪资</p>
                    <p className="font-medium text-slate-800 dark:text-slate-200">面议</p>
                  </div>
                </div>
              </div>
            </CardContent>
