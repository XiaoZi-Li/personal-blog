    ...
    <div className="dark min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
          ...
          <div className="flex gap-2 sm:gap-4">
            ...
            {project.demo && (
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="btn-neon inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm"
              >
        ...
        <div className="bg-card backdrop-blur-xl rounded-xl sm:rounded-2xl border border-white/20 p-3 sm:p-6">
          ...
          {/* 评论列表 */}
          {loading ? (
            <div className="py-6 sm:py-8 text-center text-muted-foreground text-sm">加载中...</div>
          ) : comments.length === 0 ? (
            ...
            <div className="space-y-3 sm:space-y-4">
              {comments.map((comment) => {
                ...
                return (
                  <div key={comment.id} className="bg-card rounded-lg sm:rounded-xl p-3 sm:p-4 border border-white/10">
                    {/* 主评论 */}
                    <div className="flex gap-2 sm:gap-3">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-[var(--glow-violet)] to-[var(--glow-violet)] flex items-center justify-center text-slate-900 font-medium flex-shrink-0 text-xs sm:text-sm">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 sm:gap-4">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 text-xs sm:text-sm text-muted-foreground hover:text-[var(--neon-violet)] transition-colors"
                          >
                          ...
                          {user?.isAdmin && (
                            <button
                              ...
                              className="flex items-center gap-1 text-xs sm:text-sm text-muted-foreground hover:text-red-400 transition-colors"
                            >
                    ...
                    {commentReplies.length > 0 && (
                      <div className="ml-8 sm:ml-13 mt-3 space-y-2">
                        {commentReplies.map((reply) => (
                          <div key={reply.id} className="flex gap-2 text-xs sm:text-sm bg-card rounded-lg p-2.5 sm:p-3">
                            <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-gradient-to-br from-[var(--glow-lime)] to-[var(--glow-lime)] flex items-center justify-center text-slate-900 text-[10px] sm:text-xs font-medium flex-shrink-0">
                              {reply.nickname.charAt(0).toUpperCase()}
                            </div>
                            <div className="flex-1">
                              <span className="font-medium text-muted-foreground">{reply.nickname}</span>
                              ...
                              <span className="text-muted-foreground ml-2 text-[10px] sm:text-xs">{formatDate(reply.created_at)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
