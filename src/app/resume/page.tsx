    ...
    case 'en-US':
      return {
        ...
        practices: [
          ...
          { title: 'FPGA-based Edge Intelligent Vision Terminal', role: 'System Integration & Technical Report', period: '2025/09 - 2025/12', desc: '8th National Embedded Chip & System Design Competition, FPGA Track — National 3rd Prize. Hardware acceleration of edge detection, image filtering and HSV recognition on Anlu HX4S20 FPGA, with a multi-stage pipeline supporting 640×480@30fps. I delivered system integration and wrote the full competition technical report (solution rationale, architecture description, timing and resource analysis); able to read Verilog logic and understand multi-clock-domain timing and resource constraints (LUT / BRAM / DSP optimised to 54.84% / 46.88% / 27.59%)' },
          { title: 'Quadruped Robot Dog Perception & Motion Control', role: 'Perception & Motion Control Developer', period: '2026/04 - 2026/07', desc: '9th National Embedded Chip & System Design Competition, Chip Application Track — National 3rd Prize. Built stereo depth obstacle avoidance, MediaPipe gesture control and a 3-level motion arbiter on RDK X5 (BPU 10 TOPS) + ROS2, completed gait tuning and system integration' },
          { title: 'ESP32-S3 XiaoZhi AI MCP Control System', role: 'Board Design / Firmware & Cloud Integration', period: '2025/03 - 2025/05', desc: 'China College Student Service Outsourcing Competition — National 3rd Prize | Tianjin "New Engineering" Competition — Provincial 1st Prize. Independently designed an ESP32-S3 4-layer minimal system board in LCEDA (schematic → PCB → fabrication → soldering → full-board bring-up); delivered HarmonyOS Hi3861 board-level peripheral control and sensor integration; started with ASR keyword matching for voice control, then iterated to a self-built MCP server with LLM intent recognition and tool calling, reporting device data to the cloud over MQTT' },
        ],
        passions: [
    ...
    case 'ja-JP':
      return {
        ...
        practices: [
          ...
          { title: 'FPGA ベースのエッジインテリジェントビジョン端末', role: 'システム連調・技術報告書執筆', period: '2025/09 - 2025/12', desc: '第8回全国組み込みチップ・システム設計大会 FPGA トラック — 国家級三等賞。安路 HX4S20 FPGA でエッジ検出・画像フィルタリング・HSV 認識をハードウェア高速化、多段パイプラインで 640×480@30fps リアルタイム処理を実現。本人はシステム連調と技術報告書の執筆を担当し、競技技術報告書の全文（方案論証・アーキテクチャ説明・タイミングとリソース分析）を独立完成。Verilog ロジックを読解でき、マルチクロックドメイン設計とタイミング／リソース制約を理解（LUT / BRAM / DSP を 54.84% / 46.88% / 27.59% まで最適化）' },
          { title: '四足ロボットドッグの知覚・運動制御システム', role: '知覚・運動制御開発', period: '2026/04 - 2026/07', desc: '第9回全国組み込みチップ・システム設計大会 チップ応用トラック — 国家級三等賞。RDK X5（BPU 10 TOPS）+ ROS2 で双目深度障害回避、MediaPipe ジェスチャー制御、3段優先度運動アービタを構築、歩容調整とシステム統合を完了' },
          { title: 'ESP32-S3 小智 AI MCP 制御システム', role: '基板設計 / ファームウェアと端雲連携', period: '2025/03 - 2025/05', desc: '中国大学生サービス外包イノベーション大会 — 国家級三等賞｜天津「新工科」大会 — 省部級一等賞。嘉立創 EDA で ESP32-S3 四層最小システム基板を独立設計（回路図 → PCB → 基板製造 → はんだ付け → 全基板点灯）；鴻蒙 Hi3861 のボード側ペリフェラル制御とセンサー接続を担当。当初は ASR キーワードマッチングで音声制御を実現し、後に自前の MCP サーバー＋LLM 意図認識とツール呼び出しへ発展、デバイスデータは MQTT でクラウドへ送信' },
        ],
        passions: [
        ...
        selfEval: 'エッジ側 AI 配備と組み込みソフト・ハードの交差領域に注力し、「ハードウェア-ソフトウェア-アルゴリズム」の実践チェーンを備える。RDK X5（BPU 10 TOPS）上で双目深度モデルのエッジ配備とリアルタイム推論経路を完成させ、ESP32-S3 四層最小システム基板を回路図・PCB・基板製造・はんだ付け・全基板点灯まで独立して遂行。ROS2、MCP、MQTT に精通。SCI ジャーナル Pattern Analysis and Applications に共第一著者として論文を発表し、消融実験の設計・結果分析・論文執筆を担当。組み込み競技で国家級受賞 2 件、実装力が高く、AI コーディングツールを活用した開発を得意とする。',
        educationLabel: '学歴',
    ...
    default: // cn
      return {
        ...
        practices: [
          ...
          { title: '基于 FPGA 的边缘智能视觉终端', role: '系统联调与技术报告撰写', period: '2025/09 - 2025/12', desc: '第八届全国大学生嵌入式芯片与系统设计竞赛 FPGA 创新设计赛道 · 国家级三等奖。基于安路 HX4S20 FPGA 实现边缘检测、图像滤波、HSV 色彩识别算法硬件加速，多级流水线支持 640×480@30fps 实时处理；本人承担系统联调与技术报告撰写，独立完成竞赛技术报告全文（方案论证、架构说明、时序与资源数据分析），能读懂 Verilog 逻辑并理解多时钟域设计与时序 / 资源约束（LUT / BRAM / DSP 优化至 54.84% / 46.88% / 27.59%）' },
          { title: '四足机器狗感知与运动控制系统', role: '感知与运动控制系统开发', period: '2026/04 - 2026/07', desc: '第九届全国大学生嵌入式芯片与系统设计竞赛芯片应用赛道 · 国家级三等奖。基于 RDK X5（BPU 10 TOPS）+ ROS2 开发双目深度避障、MediaPipe 手势控制与三级优先级运动仲裁器，完成步态调优与系统集成调试' },
          { title: 'ESP32-S3 小智 AI MCP 控制系统', role: '板级设计 / 板端开发与端云集成', period: '2025/03 - 2025/05', desc: '中国大学生服务外包创新创业大赛 · 国家级三等奖｜天津市"新工科"竞赛 · 省级一等奖。在嘉立创 EDA 独立完成 ESP32-S3 四层最小系统板（原理图 → PCB → 打样 → 焊接 → 整板点亮），完成鸿蒙 Hi3861 板端外设控制与传感器接入；先以 ASR 关键词匹配实现语音控制，后续迭代为自建 MCP 服务端 + 大模型意图识别与工具调用，设备数据经 MQTT 上报云平台' },
        ],
        passions: [
        ...
        selfEval: '聚焦端侧 AI 部署与嵌入式软硬件交叉领域，具备「硬件-软件-算法」的完整实践链路：在 RDK X5（BPU 10 TOPS）上完成双目深度模型的端侧部署并打通实时推理链路，独立完成 ESP32-S3 四层最小系统板从原理图、PCB 到打样焊接与整板点亮的全过程，熟悉 ROS2 机器人框架及 MCP、MQTT 协议；以共同第一作者在 SCI 期刊 Pattern Analysis and Applications 发表论文一篇，负责消融实验设计、结果分析与论文撰写；两届嵌入式竞赛国家级获奖，动手能力强，善用 AI 编程工具提升开发效率。',
        educationLabel: '教育经历',
